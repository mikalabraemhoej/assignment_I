...
def prepare_hh_ss(model):
    ...
    # a. beta
    par.beta_grid[:] = np.array([par.beta_mean-par.beta_delta,par.beta_mean, par.beta_mean+par.beta_delta,par.beta_mean-par.beta_delta,par.beta_mean, par.beta_mean+par.beta_delta]) 
    ...
    # a. raw value
    y0 = ss.w0*ss.phi0* par.z_grid    # W SLETTET
    y1 = ss.w1*ss.phi1* par.z_grid 

    c = m = (1+ss.r)*par.a_grid[np.newaxis,:] + y0[:,np.newaxis] +y1[:,np.newaxis]
...
def obj_ss(K_ss,model,do_print=False):
    ...
    ss.A = ss.K = K_ss
    ss.L0 = np.sum(ss.phi0*ss.Dbeg[:3,:,:])
    ss.L1 = np.sum(ss.phi1*ss.Dbeg[3:,:,:])
    ss.Y = ss.Gamma *  (ss.K**(par.alpha)) * (ss.L0**((1-par.alpha)/2)) * (ss.L1**((1-par.alpha)/2))
    
    # b. implied prices
    ss.rK = par.alpha*ss.Gamma *  (ss.K**(par.alpha-1)) * (ss.L0**((1-par.alpha)/2)) * (ss.L1**((1-par.alpha)/2)) # par.alpha*ss.Gamma*(ss.K/ss.L)**(par.alpha-1.0)
    ss.r = ss.rK - par.delta
    ss.w0 = ((1.0-par.alpha)/2)*(ss.Gamma*(ss.K)**par.alpha) *  (ss.L0**(-((1+par.alpha)/2))) *  (ss.L1**((1-par.alpha)/2))
    ss.w1 = ((1.0-par.alpha)/2)*(ss.Gamma*(ss.K)**par.alpha) *  (ss.L1**(-((1+par.alpha)/2))) *  (ss.L0**((1-par.alpha)/2)) 
